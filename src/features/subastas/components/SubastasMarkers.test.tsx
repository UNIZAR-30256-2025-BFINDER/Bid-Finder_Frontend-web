import React from 'react';
import { render, screen, waitFor } from '@testing-library/react';
import { describe, it, expect, beforeEach, vi } from 'vitest';
import { SubastasMarkers } from './SubastasMarkers';

const mocks = vi.hoisted(() => ({
  navigate: vi.fn(),
  fetchSubastas: vi.fn(),
}));

vi.mock('../services/subastasService', () => ({
  fetchSubastas: () => mocks.fetchSubastas(),
}));

vi.mock('./subastasIcons', () => ({
  getSubastaIcon: vi.fn(() => ({})),
}));

vi.mock('../../map/components/mapConstants', () => ({
  createYellowClusterIcon: vi.fn(),
}));

vi.mock('../../map/components/popup/subastaPopupCard', () => ({
  AuctionCard: ({ title }: { title: string }) => <div>{title}</div>,
}));

vi.mock('react-router-dom', () => ({
  useNavigate: () => mocks.navigate,
}));

vi.mock('react-leaflet-cluster', () => ({
  default: ({ children }: { children: React.ReactNode }) => (
    <div data-testid="cluster-group">{children}</div>
  ),
}));

vi.mock('react-leaflet', () => ({
  Marker: ({ children }: { children: React.ReactNode }) => (
    <div data-testid="marker">{children}</div>
  ),
  Popup: ({ children }: { children: React.ReactNode }) => (
    <div data-testid="popup">{children}</div>
  ),
}));

describe('SubastasMarkers', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('renderiza las subastas obtenidas del servicio', async () => {
    mocks.fetchSubastas.mockResolvedValue([
      {
        id: 1,
        name: 'Subasta Madrid',
        lat: 40.4,
        lng: -3.7,
        type: 'house',
        viabilidad: 'alta',
        precioActual: 100000,
        valorSubasta: 150000,
      },
      {
        id: 2,
        name: 'Subasta Sevilla',
        lat: 37.3,
        lng: -5.9,
        type: 'car',
        viabilidad: 'media',
        precioActual: 5000,
        valorSubasta: 7000,
      },
    ]);

    render(<SubastasMarkers />);

    await waitFor(() => {
      expect(screen.getByText('Subasta Madrid')).toBeInTheDocument();
      expect(screen.getByText('Subasta Sevilla')).toBeInTheDocument();
    });

    expect(screen.getByTestId('cluster-group')).toBeInTheDocument();
  });
});