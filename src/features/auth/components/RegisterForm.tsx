...
export const RegisterForm: React.FC = () => {
  ...
  const isComplete = form.name && form.email && form.password && consent.terms && consent.privacy;
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (!isComplete || hasErrors) {
      setSubmitError('Revisa los campos y acepta los términos antes de continuar.');
      return;
    }
  ...
  return (
    ...
    >
      <div className="mb-8 text-center">
        ...
        <p className="mt-2 text-sm text-gray-400">Start finding smarter auction opportunities</p>
      </div>

      <div className="space-y-5">
        <div>
          <label className="mb-2 block text-sm font-medium text-yellow-400">Alias (Público)</label>
          <input
            type="text"
            placeholder="Inversor24"
            value={form.name}
            onChange={(e) => handleChange('name', e.target.value)}
            className={`w-full rounded-md border bg-black px-4 py-3 text-sm text-white outline-none transition focus:ring-2 focus:ring-yellow-400 ${
              errors.name ? 'border-red-500' : 'border-yellow-400/70'
            }`}
          />
        ...
        <div>
          ...
          <input
            ...
            className={`w-full rounded-md border bg-black px-4 py-3 text-sm text-white outline-none transition focus:ring-2 focus:ring-yellow-400 ${
              errors.email ? 'border-red-500' : 'border-yellow-400/70'
            }`}
          />
        ...
        <div>
          ...
          <input
            ...
            className={`w-full rounded-md border bg-black px-4 py-3 text-sm text-white outline-none transition focus:ring-2 focus:ring-yellow-400 ${
              errors.password ? 'border-red-500' : 'border-yellow-400/70'
            }`}
          />
        ...
        <div className="space-y-3 mt-4">
          <label className="flex items-start gap-3 text-sm text-gray-300">
            <input
              type="checkbox"
              className="mt-1 h-4 w-4 rounded border-gray-600 bg-black text-yellow-400 focus:ring-yellow-400"
              checked={consent.terms}
              onChange={(e) => setConsent({ ...consent, terms: e.target.checked })}
            />
            <span>
              He leído y acepto los{' '}
              <Link to="/terminos" target="_blank" className="text-yellow-400 hover:underline">
          ...
          <label className="flex items-start gap-3 text-sm text-gray-300">
            ...
            />
            <span>
              Consiento el tratamiento de mis datos según la{' '}
              <Link to="/privacidad" target="_blank" className="text-yellow-400 hover:underline">
