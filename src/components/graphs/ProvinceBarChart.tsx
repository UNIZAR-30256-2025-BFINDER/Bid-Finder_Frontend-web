...
import { getStatsProvincias, StatsProvincia } from '../../features/dashboard/services/statsService';
...
const ProvinceBarChart: React.FC = () => {
  ...
  if (zonas.length === 0) {
    return (
      <div className="h-full p-5 sm:p-6 bg-[#0A0D14] border border-white/5 rounded-2xl shadow-md">
        <h2 className="text-lg sm:text-xl font-semibold text-white mb-4">Top 10 Zonas Activas</h2>
  ...
  const options: ChartOptions<'bar'> = {
    ...
    plugins: {
      ...
      tooltip: {
        callbacks: {
          label: (tooltipItem: TooltipItem<'bar'>) => `Subastas: ${tooltipItem.raw}`,
        },
      },
    },
    scales: {
